...
const SIRI_FALLBACK = "I can help navigate Dung's portfolio. Try asking me to open Finder, Terminal, Resume, VS Code, GitHub, or check the time.";

const SYSTEM_PROMPT = `You are a friendly virtual assistant running inside Dung Dương's macOS-style web portfolio.
You control the interface through tool calls. Keep replies short, warm, and conversational (1-2 sentences max).
Use a casual, friendly tone. Say things like "Sure thing!", "You got it!", "No worries!", "Here you go!".

Here is info about Dung (use this when someone asks about him or his resume):
${DUNG_INFO}

IMPORTANT RULES:
1. If you don't understand the request, or it is unrelated to the portfolio, reply EXACTLY: "${SIRI_FALLBACK}"
2. If asked about Dung, his skills, experience, resume, or background, ALWAYS call the download_resume tool and share a brief summary focused on backend systems, WebAssembly, multimedia, identity, and distributed systems.
...
6. You can open the launchpad to show Dung's projects, open Terminal, Finder, Resume, VS Code, toggle fullscreen, and more.
7. Never say "I am Dung". You are an assistant inside Dung's portfolio.`;
...
const TOOLS = [
  ...
  { type: "function", function: { name: "open_app", description: "Opens an app window.", parameters: { type: "object", properties: { app_id: { type: "string", description: "One of: finder, terminal, vscode, typora, bear, safari, mail, messages, photos, notes, music, system-settings" } }, required: ["app_id"] } } },
  { type: "function", function: { name: "close_app", description: "Closes an app window.", parameters: { type: "object", properties: { app_id: { type: "string", description: "One of: finder, terminal, vscode, typora, bear, safari, mail, messages, photos, notes, music, system-settings" } }, required: ["app_id"] } } },
  ...
  { type: "function", function: { name: "download_resume", description: "Downloads Dung's resume/profile PDF. Call this whenever the user asks about Dung, his background, skills, experience, or wants to see his resume." } },
  { type: "function", function: { name: "open_launchpad", description: "Opens the Launchpad overlay to show Dung's projects." } },
  { type: "function", function: { name: "open_music", description: "Opens the Music app." } }
];
...
export default function Siri({ closeSiri }: { closeSiri?: () => void }) {
  ...
  const downloadResume = useCallback(() => {
    ...
    link.download = "Dung_Duong_Resume.pdf";
    link.target = "_blank";
  ...
  const executeTool = useCallback(async (name: string, args: any): Promise<string> => {
    ...
    switch (name) {
      ...
      case "download_resume":
        downloadResume();
        return "Here's Dung's resume and profile. The download should start now.";
      ...
      case "open_launchpad": {
        ...
        return "Opening the Launchpad with Dung's project shortcuts.";
      }

      case "open_music": {
        openAppById("music");
        return "Opening the Music app.";
      }
