...
const Window = (props: WindowProps) => {
  ...
  const initialXOffset = (props.x || 0) + (props.xPercent || 0) * winWidth;

  const [state, setState] = useState<WindowState>({
    width: initWidth,
    height: initHeight,
    x: (winWidth - initWidth) / 2 + initialXOffset,
  ...
  const height = (props.max || isMobile) ? (isMobile ? winHeight : winHeight - minMarginY) : state.height;
  const disableMax = props.aspectRatio !== undefined;
  const maxX = Math.max(minMarginX, winWidth - state.width - minMarginX);
  const maxY = Math.max(0, winHeight - minMarginY - (dockSize + 15 + minMarginY));
  const safeTop = props.max || isMobile ? 0 : 12;
    ...
    <Rnd
      ...
      position={{
        x: (props.max || isMobile)
          ? 0
          : Math.min(maxX, Math.max(minMarginX, state.x)),
        y: (props.max || isMobile)
          ? 0
          : Math.min(maxY, Math.max(safeTop, state.y)),
      }}
      ...
      className={`${round} ${minimized}`}
      id={`window-${props.id}`}
    >
      {/* macOS desktop window shell */}
      <motion.div
